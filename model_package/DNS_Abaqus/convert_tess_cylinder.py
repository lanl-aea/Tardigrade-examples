...
def parse_contents(input_file):
    ...
    # # collect bounding faces
    # bound_start = bound_index[0]+2
    # num_bounds = 6
    # desired_lines = list(input_contents[bound_start:bound_start+num_bounds*4])
    # line_to_grab = 1
    # for face_id in range(0, num_bounds):
        # items = re.split(r'\s+', desired_lines[face_id*4 + line_to_grab])
        # #normal = re.split(r'\s+', desired_lines[face_id*4 + 3])
        # bounds[face_id+1] = [str(abs(int(i))) for i in items[2:]]
    ...
    for line in input_contents[poly_start:poly_start+num_poly]:
        items = re.split(r'\s+', line)
        #poly = numpy.unique(numpy.array([abs(int(i)) for i in items[2:]]))
...
def create_mesh(stl_file, mesh_file, polyhedra, sidesets):
    ...
    for sideset in sidesets.keys():
        cubit.cmd(f'nodeset {sideset_num} add surface {sidesets[sideset]}')
        cubit.cmd(f'nodeset {sideset_num} name "{sideset}"')
        cubit.cmd(f'sideset {sideset_num} add surface {sidesets[sideset]}')
...
def convert_tess(input_file, stl_file=None, mesh_file=None, seed_size=1.0):
    ...
    sidesets = {'top': ' with z_min > 0.99',
                'bottom': 'with z_max < 0.01',
                'side': 'all patch cylinder axis direction 0 0 1 origin 0.5 0.5 0 outer_radius 0.51 inner_radius 0.49',}
