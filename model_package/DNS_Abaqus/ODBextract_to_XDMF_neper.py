        ...
        for f in frames:
            ...
            if collocation_option == 'ip':
                if element_type != 'C3D8':
                    print(f'WARNING! Interpolation for {element_type} has not been implemented!')
                u_elem = interpolate_to_ip_c3d8(u_nodes, mesh)
                U1, U2, U3 = u_elem[:,:,0].flatten(order='C'), u_elem[:,:,1].flatten(order='C'), u_elem[:,:,2].flatten(order='C')
                if velocities:
                    v_elem = interpolate_to_ip_c3d8(v_nodes, mesh)
                    V1, V2, V3 = v_elem[:,:,0].flatten(order='C'), v_elem[:,:,1].flatten(order='C'), v_elem[:,:,2].flatten(order='C')
                else:
                    V1, V2, V3 = null, null, null
                if accelerations:
                    a_elem = interpolate_to_ip_c3d8(a_nodes, mesh)
...
def initialize_reference(old_results, old_times, input_file, mesh_path, collocation_option):
    ...
    with h5py.File(input_file, 'r') as file:
        node = np.array(file[mesh_path]['node'])
        node_location = np.array(file[mesh_path]['node_location'])
        mesh = np.array(file[mesh_path][f'C3D4_mesh'])
        if collocation_option == 'ip':
            print(f'WARNING! Interpolation for C3D4 has not been implemented!')
        elif collocation_option == 'center':
            COORD = interpolate_to_center(node_location, mesh)
            volume = c3d4_volume(node_location, mesh)
        else:
            print('Specify valid collocation options')
        results[0]['COORDx'] = COORD[:,0]
        results[0]['COORDy'] = COORD[:,1]
        results[0]['COORDz'] = COORD[:,2]
        results[0]['IVOL'] = volume
        
    # Setup initial frame for all other fields
    results[0]['time'] = 0.0
    zero_fields = ['S11', 'S22', 'S33', 'S12', 'S13', 'S23', 'U1', 'U2', 'U3', 'V1', 'V2', 'V3', 'A1', 'A2', 'A3']
