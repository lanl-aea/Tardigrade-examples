...
def build_input(output_file, mesh_file, parameter_sets, platen_radius,
    ...
    with open(output_file, 'w') as f:
        ...
        # Kernels
        f.write('# Kernels\n')
        moose_tools.write_kernels(f, phis)
        f.write('[]\n')
        f.write('\n')
        ...
        if extra_stress_output is not None:
            moose_tools.write_extra_second_order_stress_auxkernels(f)
        if higher_order_stress_output == 'on':
        ...
        # NodalKernels
        f.write('[NodalKernels]\n')
        f.write('  [./bottom_y]\n')
        f.write('    type = CylindricalSurfaceNodalContact\n')
        ...
        if len(parameter_sets) > 1:
            for i, set in enumerate(parameter_sets):
                ...
                stream.close()
                if material_type == 'plastic':
                    moose_tools.write_plastic_material_card(f, UI, i)
                else:
            ...
            stream.close()
            if material_type == 'plastic':
                moose_tools.write_plastic_material_card(f, UI)
            else:
...
def get_parser():
    ...
    cli_description = "Write MOOSE input file for Brazilian disk simulation with nodal kernel contact"
